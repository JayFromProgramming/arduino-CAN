...
MCP2515Class::MCP2515Class() : CANControllerClass(),
...
int MCP2515Class::parsePacket() {
    ...
    if (_rxExtended) {
        uint32_t idB = (((uint32_t) (readRegister(REG_RXBnSIDL(n)) & 0x03) << 16) & 0x30000) | (
                           (readRegister(REG_RXBnEID8(n)) << 8) & 0xff00) | readRegister(REG_RXBnEID0(n));
